...
def web_main():
                ...
                if snapshot is None:
                    ...
                    try:
                        ...
                        if not target.is_file():
                            fallback_names = ("final_preview.png", "mesh.png", "cleaned.png", "original.png")
                            target = next((target.parent / name for name in fallback_names if (target.parent / name).is_file()), target)
                        if not target.is_file():
                            raise FileNotFoundError(filename)
    ...
    WEB_PAGE = WEB_PAGE.replace(
        "</section></div></main>",
        """</section></div><section class="bar"><h2>3D Model Viewer</h2><div id="modelLabel">Upload and process designs to view their 3D models.</div><select id="designSelect" onchange="selectDesign(this.value)"><option>No designs</option></select><div id="modelViewer" style="height:420px;background:#17231f;position:relative;overflow:hidden"></div><button onclick="previousDesign()">Previous Design</button><button onclick="nextDesign()">Next Design</button><span id="designCounter">0 / 0</span><h2>Exports</h2><div id="outputs" class="outputs">Select a completed design to see its files.</div></section></main>""",
    )
    WEB_PAGE = WEB_PAGE.replace(
        "</body></html>",
        """<script src="https://cdn.jsdelivr.net/npm/three@0.128.0/build/three.min.js"></script><script>
let designIndex=0,modelScene,modelCamera,modelRenderer,modelObject;
function setupModelViewer(){const box=document.getElementById('modelViewer');if(!window.THREE||!box)return;modelScene=new THREE.Scene();modelScene.background=new THREE.Color(0x17231f);modelCamera=new THREE.PerspectiveCamera(45,box.clientWidth/box.clientHeight,.1,5000);modelCamera.position.set(0,0,140);modelRenderer=new THREE.WebGLRenderer({antialias:true});modelRenderer.setPixelRatio(Math.min(devicePixelRatio,2));modelRenderer.setSize(box.clientWidth,box.clientHeight);box.appendChild(modelRenderer.domElement);modelScene.add(new THREE.HemisphereLight(0xffffff,0x34544a,2));const light=new THREE.DirectionalLight(0xffffff,2);light.position.set(80,100,120);modelScene.add(light);function animate(){requestAnimationFrame(animate);if(modelObject){modelObject.rotation.y+=0.006}modelRenderer.render(modelScene,modelCamera)}animate();window.addEventListener('resize',()=>{modelCamera.aspect=box.clientWidth/box.clientHeight;modelCamera.updateProjectionMatrix();modelRenderer.setSize(box.clientWidth,box.clientHeight)});}
function parseOBJ(text){const vertices=[],positions=[],lines=text.split(String.fromCharCode(10));for(const line of lines){const parts=line.trim().split(/[ \t]+/);if(parts[0]==='v')vertices.push(parts.slice(1,4).map(Number));if(parts[0]==='f'){const face=parts.slice(1).map(part=>parseInt(part.split('/')[0],10)-1);for(let i=1;i<face.length-1;i++){for(const index of [face[0],face[i],face[i+1]])positions.push(...vertices[index])}}}const geometry=new THREE.BufferGeometry();geometry.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometry.computeVertexNormals();return new THREE.Mesh(geometry,new THREE.MeshStandardMaterial({color:0x55b7a0,metalness:.15,roughness:.65}));}
function loadDesign(index){if(!currentJob||!files[index]||!modelScene)return;designIndex=(index+files.length)%files.length;document.getElementById('modelLabel').textContent=files[designIndex].name+' / 3D model';document.getElementById('designCounter').textContent=(designIndex+1)+' / '+files.length;if(modelObject)modelScene.remove(modelObject);fetch('/api/jobs/'+currentJob+'/assets/'+designIndex+'/model.obj?t='+Date.now()).then(response=>{if(!response.ok)throw new Error('OBJ unavailable');return response.text()}).then(text=>{modelObject=parseOBJ(text);const box=new THREE.Box3().setFromObject(modelObject),center=box.getCenter(new THREE.Vector3()),size=box.getSize(new THREE.Vector3()),scale=80/Math.max(size.x,size.y,size.z,1);modelObject.position.sub(center);modelObject.scale.setScalar(scale);modelScene.add(modelObject)}).catch(()=>{document.getElementById('modelLabel').textContent=files[designIndex].name+' / 3D model not ready yet';});}
function previousDesign(){loadDesign(designIndex-1)}function nextDesign(){loadDesign(designIndex+1)}window.addEventListener('load',()=>{setupModelViewer();setInterval(()=>{if(currentJob&&modelScene&&files.length&&!modelObject)loadDesign(0)},500)});
</script></body></html>""",
    )
    WEB_PAGE = WEB_PAGE.replace(
        "</body></html>",
        """<script>
let selectedDesign=0,viewerCanvas,viewerContext,viewerModel;
function setupModelViewer(){const box=document.getElementById('modelViewer');if(!box)return;viewerCanvas=document.createElement('canvas');viewerCanvas.width=box.clientWidth||640;viewerCanvas.height=box.clientHeight||420;viewerCanvas.style.width='100%';viewerCanvas.style.height='100%';box.replaceChildren(viewerCanvas);viewerContext=viewerCanvas.getContext('2d');drawViewer();}
function parseViewerOBJ(text){const vertices=[],faces=[];for(const line of text.split(String.fromCharCode(10))){const parts=line.trim().split(/[ \t]+/);if(parts[0]==='v')vertices.push(parts.slice(1,4).map(Number));if(parts[0]==='f')faces.push(parts.slice(1).map(part=>parseInt(part.split('/')[0],10)-1));}return {vertices,faces};}
function drawViewer(){if(!viewerContext)return;viewerContext.fillStyle='#17231f';viewerContext.fillRect(0,0,viewerCanvas.width,viewerCanvas.height);if(!viewerModel){viewerContext.fillStyle='#b7ccc4';viewerContext.font='16px system-ui';viewerContext.fillText('Process a design to view its 3D model',24,40);return;}const points=viewerModel.vertices;let minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity,minZ=Infinity,maxZ=-Infinity;points.forEach(p=>{minX=Math.min(minX,p[0]);maxX=Math.max(maxX,p[0]);minY=Math.min(minY,p[1]);maxY=Math.max(maxY,p[1]);minZ=Math.min(minZ,p[2]);maxZ=Math.max(maxZ,p[2]);});const scale=Math.min(viewerCanvas.width/Math.max(maxX-minX,1),viewerCanvas.height/Math.max(maxY-minY,1))*.72;const project=p=>[viewerCanvas.width/2+(p[0]-(minX+maxX)/2)*scale+(p[2]-(minZ+maxZ)/2)*scale*.35,viewerCanvas.height/2-(p[1]-(minY+maxY)/2)*scale-(p[2]-(minZ+maxZ)/2)*scale*.35];viewerContext.lineWidth=1;viewerModel.faces.forEach(face=>{const projected=face.map(index=>project(points[index]));viewerContext.beginPath();projected.forEach((p,i)=>i?viewerContext.lineTo(p[0],p[1]):viewerContext.moveTo(p[0],p[1]));viewerContext.closePath();viewerContext.fillStyle='#4c9e8b';viewerContext.fill();viewerContext.strokeStyle='#a6d8ca';viewerContext.stroke();});}
function loadDesign(index){if(!currentJob||!files.length)return;selectedDesign=(index+files.length)%files.length;document.getElementById('designCounter').textContent=(selectedDesign+1)+' / '+files.length;document.getElementById('modelLabel').textContent=files[selectedDesign].name+' / 3D model';showSelectedPreview();fetch('/api/jobs/'+currentJob+'/assets/'+selectedDesign+'/model.obj?t='+Date.now()).then(response=>{if(!response.ok)throw new Error('OBJ unavailable');return response.text()}).then(text=>{viewerModel=parseViewerOBJ(text);drawViewer()}).catch(()=>{document.getElementById('modelLabel').textContent=files[selectedDesign].name+' / 3D model not ready yet';});}
function showSelectedPreview(){if(!currentJob||!files.length)return;const node=document.getElementById('s'+selectedDesign);const stage=node&&node.textContent.includes('/')?node.textContent.split('/').slice(1).join('/').trim():'Original';const filename=stageFiles[stage]||'original.png';preview.src='/api/jobs/'+currentJob+'/assets/'+selectedDesign+'/'+filename+'?t='+Date.now();previewLabel.textContent=files[selectedDesign].name+' / '+stage;}
function previousDesign(){loadDesign(selectedDesign-1)}function nextDesign(){loadDesign(selectedDesign+1)}
function showPreview(index,stage){selectedDesign=index;loadDesign(index);showSelectedPreview();}
function selectDesign(index){selectedDesign=Number(index)||0;loadDesign(selectedDesign);showSelectedPreview();}
function renderDesignSelector(){const select=document.getElementById('designSelect');if(!select)return;select.innerHTML=files.length?files.map((file,index)=>'<option value="'+index+'">'+(index+1)+'. '+file.name+'</option>').join(''):'<option>No designs</option>';select.value=String(selectedDesign);document.getElementById('designCounter').textContent=files.length?((selectedDesign+1)+' / '+files.length):'0 / 0';}
function showOutputs(index,asset){const panel=document.getElementById('outputs');if(!panel)return;if(!asset||!asset.outputs){panel.textContent='This design has not completed yet.';return;}const archive=asset.export_archive||files[index].replace(/\\.[^.]+$/,'')+'_exports.zip';panel.innerHTML='<strong>'+files[index].name+'</strong><p>Export folder: <code>processed/'+files[index].name.replace(/\\.[^.]+$/,'')+'/</code></p><a href="/api/jobs/'+currentJob+'/assets/'+index+'/'+archive+'" target="_blank">Download complete design folder (.zip)</a><h3>Files</h3>'+asset.outputs.filter(name=>name!==archive).map(name=>'<a href="/api/jobs/'+currentJob+'/assets/'+index+'/'+name+'" target="_blank">Download '+name+'</a>').join('')+'<h3>Stage folders</h3><p>01 Original, 02 Background Removed, 03 Threshold, 04 Cleaned Mask, 05 Contours, 06 Polygon, 07 Mesh, 08 Final</p>';}
function poll(job){fetch('/api/jobs/'+job).then(response=>response.json()).then(result=>{result.assets.forEach((asset,index)=>{const node=document.getElementById('s'+index);if(node)node.textContent=asset.status+(asset.stage?' / '+asset.stage:'')+(asset.error?': '+asset.error:'');});if(result.current)live.textContent='Live: '+result.current.filename+' / '+result.current.stage;if(files.length){if(!currentJob)currentJob=job;renderDesignSelector();loadDesign(selectedDesign);showSelectedPreview();}if(result.status==='Queued'||result.status==='Processing')setTimeout(()=>poll(job),250);else{live.textContent='Batch '+result.status.toLowerCase();if(result.assets[selectedDesign])showOutputs(selectedDesign,result.assets[selectedDesign]);}}).catch(error=>{live.textContent='Batch status unavailable: '+error.message;});}
window.addEventListener('load',()=>{setupModelViewer();renderDesignSelector();});
</script></body></html>""",
    )
    WEB_PAGE = WEB_PAGE.replace(
        "</body></html>",
        """<script>
let latestAssets=[];
function exportsPanel(){const panels=document.querySelectorAll('#outputs');return panels[panels.length-1]||document.getElementById('exportPanel');}
function renderExportTabs(){let panel=exportsPanel();if(!panel)return;let tabs=document.getElementById('designExportTabs');if(!tabs){tabs=document.createElement('div');tabs.id='designExportTabs';tabs.className='outputs';panel.parentNode.insertBefore(tabs,panel);}tabs.innerHTML=files.map((file,index)=>'<button onclick="selectExportDesign('+index+')" style="font-weight:'+(index===selectedDesign?'bold':'normal')+'">'+(index+1)+'. '+file.name+'</button>').join('');}
function selectExportDesign(index){selectedDesign=Number(index)||0;renderDesignSelector();loadDesign(selectedDesign);showSelectedPreview();if(latestAssets[selectedDesign])showOutputs(selectedDesign,latestAssets[selectedDesign]);renderExportTabs();}
function showOutputs(index,asset){const panel=exportsPanel();if(!panel)return;if(!asset||!asset.outputs){panel.textContent='This design has not completed yet.';return;}const archive=asset.export_archive||files[index].replace(/\\.[^.]+$/,'')+'_exports.zip';panel.innerHTML='<strong>'+files[index].name+'</strong><p>Export folder: <code>processed/'+files[index].name.replace(/\\.[^.]+$/,'')+'/</code></p><a href="/api/jobs/'+currentJob+'/assets/'+index+'/'+archive+'" target="_blank">Download complete design folder (.zip)</a><h3>Files</h3>'+asset.outputs.filter(name=>name!==archive).map(name=>'<a href="/api/jobs/'+currentJob+'/assets/'+index+'/'+name+'" target="_blank">Download '+name+'</a>').join('')+'<h3>Stage folders</h3><p>01 Original, 02 Background Removed, 03 Threshold, 04 Cleaned Mask, 05 Contours, 06 Polygon, 07 Mesh, 08 Final</p>';renderExportTabs();}
function poll(job){fetch('/api/jobs/'+job).then(response=>response.json()).then(result=>{latestAssets=result.assets||[];result.assets.forEach((asset,index)=>{const node=document.getElementById('s'+index);if(node)node.textContent=asset.status+(asset.stage?' / '+asset.stage:'')+(asset.error?': '+asset.error:'');});if(result.current)live.textContent='Live: '+result.current.filename+' / '+result.current.stage;if(files.length){currentJob=job;renderDesignSelector();renderExportTabs();loadDesign(selectedDesign);showSelectedPreview();}if(result.status==='Queued'||result.status==='Processing')setTimeout(()=>poll(job),250);else{live.textContent='Batch '+result.status.toLowerCase();showOutputs(selectedDesign,latestAssets[selectedDesign]);}}).catch(error=>{live.textContent='Batch status unavailable: '+error.message;});}
</script></body></html>""",
    )
